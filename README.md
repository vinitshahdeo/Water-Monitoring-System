...
Water Monitoring System is an IoT based Liquid Level Monitoring system that has the mechanism to keep the user alerted in case of liquid overflow or when the tank depletes. The water tanks can be fixed with ultrasonic sensors that are placed over the container. An ultrasonic sensor is used to measure, compare container depth and liquid level.
